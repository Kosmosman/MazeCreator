...
namespace s21 {
void Field::CreateField(const Field::size_t &new_height,
                        const Field::size_t &new_width) {
  if (height_ != new_height || width_ != new_width) {
    height_ = new_height;
    width_ = new_width;
    vertexes_.resize(height_ * width_ * 4);
    float cell_h{static_cast<float>(FIELD_SIDE_SIZE) /
                 static_cast<float>(height_)};
    float cell_w{static_cast<float>(FIELD_SIDE_SIZE) /
    ...
    for (size_t i = 0; i < height_; ++i) {
      for (size_t j = 0; j < width_; ++j) {
        ...
        vertexes_[start_pos + 2].position = {(j + 1) * cell_w,
                                             (i + 1) * cell_h};
...
void Field::CreatePath(const std::vector<std::pair<size_t, size_t>> &path) {
  ...
  for (size_t i = 0, pos = 0; i < path.size() - 1; ++i) {
    ...
    auto left_x{(vertexes_[start_pos].position.x +
                 vertexes_[start_pos + 1].position.x) /
                2};
    auto right_x{
        (vertexes_[end_pos].position.x + vertexes_[end_pos + 1].position.x) /
        2};
    auto top_y{(vertexes_[start_pos + 1].position.y +
                vertexes_[start_pos + 2].position.y) /
               2};
    auto down_y{(vertexes_[end_pos + 1].position.y +
                 vertexes_[end_pos + 2].position.y) /
                2};

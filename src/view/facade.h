...
namespace s21 {
            ...
            for (auto &i: text_labels_) {
                if (i.CheckCondition()) {
                    if (sc == sf::Keyboard::Scancode::Backspace)
                        i.Remove();
                    else if (i.GetString().size() < 2 || i.GetValue() == TextTables::kValue::FILENAME)
    ...
    private:
        ...
        void Init() {
            ...
            buttons_.back().Fill({50, 530}, {132, 32}, Button::kFunctionality::FIND_WAY);

            buttons_.emplace_back("../view/font/Lato-Italic.ttf", "Generate maze (x, y)");
            buttons_.back().Fill({50, 580}, {132, 32}, Button::kFunctionality::GENERATE_MAZE);

            buttons_.emplace_back("../view/font/Lato-Italic.ttf", "Load maze from file");
            buttons_.back().Fill({50, 630}, {135, 32}, Button::kFunctionality::LOAD_MAZE);

            text_labels_.reserve(7);
            text_labels_.emplace_back("../view/font/Lato-Italic.ttf");
            text_labels_.back().Fill({200, 580}, {32, 32}, TextTables::kValue::X_COORDINATE);

            text_labels_.emplace_back("../view/font/Lato-Italic.ttf");
            text_labels_.back().Fill({250, 580}, {32, 32}, TextTables::kValue::Y_COORDINATE);

            text_labels_.emplace_back("../view/font/Lato-Italic.ttf");
            text_labels_.back().Fill({200, 630}, {250, 32}, TextTables::kValue::FILENAME);
        ...
        void GenerateHandler() {
            ...
            field_.CreateMazeGraph(controller_->GetVerticalWalls(), controller_->GetHorizontalWalls());

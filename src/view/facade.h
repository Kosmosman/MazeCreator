...
class Facade {
 public:
  Facade(Controller *controller, std::vector<Button> *buttons,
         std::vector<TextTables> *text_labels, Field *field)
      : controller_{controller},
        buttons_{buttons},
        text_labels_{text_labels},
        field_{field} {
    Init();
  };

  void MousePressed(sf::Vector2i pos) {
    for (auto &i : *text_labels_) i.ChangeCondition(false);
    for (auto &i : *buttons_) {
    ...
    for (auto &i : *text_labels_) {
      if (i.CheckPosition(pos)) LabelsHandler(pos);
    }
  }
 ...
 private:
  void Init() {
    ...
    text_labels_->emplace_back("../view/font/Lato-Italic.ttf");
    text_labels_->back().Fill({200, 530}, {32, 32},
                              TextTables::kValue::X_START);

    text_labels_->emplace_back("../view/font/Lato-Italic.ttf");
    text_labels_->back().Fill({250, 530}, {32, 32},
                              TextTables::kValue::Y_START);

...
namespace s21 {
    ...
    public:
        ...
        const std::vector<std::vector<bool>> &GetVerticalWalls() { return mg_->GetVerticalWalls(); };

        const std::vector<std::vector<bool>> &GetHorizontalWalls() { return mg_->GetHorizontalWalls(); };

        const std::vector<std::pair<size_t, size_t>> &
        GetWay(std::pair<size_t, size_t> &&start, std::pair<size_t, size_t> &&end) {
            try {
                IsValidPositions(
                        {static_cast<int>(start.first), static_cast<int>(start.second), static_cast<int>(end.first),
                         static_cast<int>(end.second)});
    ...
    private:
        ...
        void IsValidPositions(std::vector<int> &&positions) {
            try {
                Validator::CheckValidBorders(std::move(positions), static_cast<int>(GetWidth()),

...
namespace s21 {
    ...
    public:
        ...
        static void CheckValidCoordinate(int x, int y) {
            ...
            if (x > MAX_WALL_SIZE | y > MAX_WALL_SIZE) throw std::out_of_range("Over then max size");
        }

        static void CheckValidBorders(std::vector<int> &&positions, int x_border, int y_border) {
            if (positions[0] < 0 | positions[1] < 0 | positions[2] < 0 | positions[3] < 0)
                throw std::out_of_range("Less then zero");
            if (positions[0] >= x_border | positions[1] >= y_border | positions[2] >= x_border |
                positions[3] >= y_border)
